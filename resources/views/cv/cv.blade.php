...
<body>
    ...
    <div class="header">
        ...
        <p>
            ...
            @if (app()->getLocale() == 'en')
                {{$user->jobtitle->name_en}} 
            @else 
                {{$user->jobtitle->name_ar}}
            ...
            @if (app()->getLocale() == 'en')
                {{ $user->scopework->name_en }}
            @else 
                {{ $user->scopework->name_ar }}
            @endif
          
        </p>
        <p>{{ $user->phone }} | {{ $user->email }}</p>
        <p>{{ $user->city->name }} | {{ $user->nationality }}</p>
    </div>

    <!-- المعلومات الشخصية -->
    <div class="section">
        <div class="section-title">{{ __('cv.personal_information') }}</div>
        <div class="section-content">
            <p><strong>{{ __('cv.gender') }}:</strong> {{ $user->gender }}</p>
            <p><strong>{{ __('cv.birthday') }}:</strong> {{ $user->birthday }}</p>
            <p><strong>{{ __('cv.address') }}:</strong> {{ $user->address }}</p>
        </div>
    </div>

    <!-- الخبرات -->
    <div class="section">
        <div class="section-title">{{ __('cv.experiences') }}</div>
        <div class="section-content">
            @foreach ($user->experience as $experience)
                <p><strong>{{ $experience->name_company }} ({{ $experience->from_date }} - {{ $experience->to_date }}):</strong> {{ $experience->text }}</p>
            @endforeach
        </div>
    </div>
    ...
    <div class="section">
        ...
        <div class="section-content">
            @foreach ($user->skill as $skill)
                <p><strong>{{ $skill->name }}:</strong> <span class="stars">{{ str_repeat('★', $skill->rang) }}</span></p>
            @endforeach
        </div>
    </div>
    ...
    <div class="section">
        ...
        <div class="section-content">
            @foreach ($user->language as $language)
                <p><strong>{{ $language->name }}:</strong> <span class="stars">{{ str_repeat('★', $language->rang) }}</span></p>
            @endforeach
        </div>
    </div>
    ...
    <!-- الشهادات -->
    <div class="section">
        <div class="section-title">{{ __('cv.certificates') }}</div>
        <div class="section-content">
            @foreach ($user->certificate as $certificate)
                <p><strong>{{ $certificate->certificate_name }} ({{ $certificate->certificate_type }}):</strong></p>
                @if ($certificate->image)
                    <img src="{{ public_path('storage/certificates/' . $certificate->image) }}" alt="{{ $certificate->certificate_name }}" style="max-width: 100px;">
                @endif
            @endforeach
        </div>
    </div>
    ...
    <!-- معرض الأعمال -->
    <div class="section">
        <div class="section-title">{{ __('cv.business_gallery') }}</div>
        <div class="section-content">
            @foreach ($user->businessgallery as $gallery)
                <p><strong>{{ $gallery->name }}:</strong> <a href="{{ $gallery->link }}">{{ $gallery->link }}</a></p>
            @endforeach
        </div>
    </div>
    ...
    <!-- الوصف -->
    <div class="section">
        <div class="section-title">{{ __('cv.description') }}</div>
        <div class="section-content">
            <p>{{ $user->userdetails->description }}</p>
        </div>
    </div> --}}
